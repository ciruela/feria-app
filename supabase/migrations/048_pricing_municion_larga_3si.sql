-- =====================================================================
-- 048_pricing_municion_larga_3si.sql
-- ---------------------------------------------------------------------
-- 3 cuotas SI de munición solo para calibres de arma larga cuando
-- pricing.municion.tarjeta3_solo_arma_larga = true.
-- 10% efectivo/transferencia sigue aplicando a toda la munición.
-- =====================================================================

begin;

-- Espejo de lib/utils/municion_calibre.dart (heurística pistola/revólver).
create or replace function public._is_municion_arma_corta(
  p_calibre text,
...
declare
  v text;
begin
  v := lower(trim(coalesce(p_calibre, '') || ' ' || coalesce(p_descripcion, '')));
  v := translate(v, 'áéíóúñ', 'aeioun');
  v := regexp_replace(v, '[^a-z0-9.x/]+', ' ', 'g');
  ...
  -- Prefijo/sufijo de palabra para no confundir .45 con .458, .9 con .9.3, etc.
  return v ~ '(^| )(c\.?\s*)?\.?9(mm|x19|x21|x18| para|mm para)?( |$)'
      or v ~ '(^| )9\s*mm( |$)'
      or v ~ '(^| )9\s*x\s*19( |$)'
      or v ~ '(^| )(c\.?\s*)?\.?380(acp| auto)?( |$)'
      or v ~ '(^| )9\s*corto( |$)'
      or v ~ '(^| )(c\.?\s*)?\.?45(\s*(acp|auto))?(\.0+)?( |$)'
      or v ~ '(^| )(c\.?\s*)?\.?40(\s*(s.?w|sw))?(\.0+)?( |$)'
      or v ~ '(^| )(c\.?\s*)?\.?38(\s*special|\s*spl)?(\.0+)?( |$)'
      or v ~ '(^| )(c\.?\s*)?\.?357(\s*mag(num)?)?(\.0+)?( |$)'
      or v ~ '(^| )(c\.?\s*)?\.?44(\s*(mag(num)?|special|spl))?(\.0+)?( |$)'
      or v ~ '(^| )(c\.?\s*)?\.?32(\s*(acp|auto))?(\.0+)?( |$)'
      or v ~ '(^| )7\.65( |$)'
      or v ~ '(^| )(c\.?\s*)?\.?25(\s*(acp|auto))?(\.0+)?( |$)'
      or v ~ '(^| )6\.35( |$)'
      or v ~ '(^| )10\s*mm( |$)'
      or v ~ '(^| )(c\.?\s*)?\.?41(\s*mag(num)?)?(\.0+)?( |$)'
      or v ~ '(^| )(c\.?\s*)?\.?50(\s*(ae|action express))( |$)';
end;
$$;

create or replace function public._is_municion_arma_larga(
  p_calibre text,
  p_descripcion text default ''
) returns boolean
language sql
immutable
as $$
  select not public._is_municion_arma_corta(p_calibre, p_descripcion);
$$;

revoke all on function public._is_municion_arma_corta(text, text)
  from public, anon, authenticated;
revoke all on function public._is_municion_arma_larga(text, text)
  from public, anon, authenticated;

drop function if exists public._sale_unit_ars(numeric, numeric, text, jsonb, text);
...
begin
  ...
  if v_mun is not null then
    ...
    if v_t3_solo_larga
      ...
      -- Munición corta: recargo 3 cuotas global.
...
-- register_sale: pasar calibre + descripcion a _sale_unit_ars.
...
-- Flag en World Guns (idempotente).
update public.app_config ac
   set pricing_settings = jsonb_set(
         coalesce(ac.pricing_settings, '{}'::jsonb),
         '{municion,tarjeta3_solo_arma_larga}',
         'true'::jsonb,
         true
       ),
