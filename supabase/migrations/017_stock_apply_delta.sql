-- Descuento / restitución de stock atómico (productos + movimiento en una transacción).
-- Evita ventas sin movimiento o stock desincronizado entre dispositivos.
...
begin
  if p_delta = 0 then
    select stock into v_current from public.productos where id = p_product_id;
    if not found then
