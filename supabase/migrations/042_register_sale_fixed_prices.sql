-- =====================================================================
-- 042_register_sale_fixed_prices.sql
-- ---------------------------------------------------------------------
-- Urban Tactical (y otros tenants con productos.fixed_prices): register_sale
-- debe cotizar igual que la app (PricingService._fromFixed).
--
-- Antes: la RPC solo usaba precio_usd + tipo de cambio. En Urban precio_usd
-- suele ser 0 y el total real está en fixed_prices → server total_ars ≈ 0,
-- el cliente ve ARS reales, saleTotalsDiverge anula la venta con el mensaje
-- engañoso "El precio cambió mientras se confirmaba la venta".
-- =====================================================================

begin;

-- Espejo de FixedPrices.isEmpty / PricingService._fromFixed (lib/).
create or replace function public._sale_has_fixed_prices(p_fixed jsonb)
returns boolean
language sql
immutable
as $$
  select p_fixed is not null
    and (
      coalesce((p_fixed->>'efectivo_ars')::numeric, 0) > 0
      or coalesce((p_fixed->>'tarjeta_ars')::numeric, 0) > 0
      or coalesce((p_fixed->>'cuota3_ars')::numeric, 0) > 0
      or coalesce((p_fixed->>'cuota6_ars')::numeric, 0) > 0
      or coalesce((p_fixed->>'cuota12_ars')::numeric, 0) > 0
      or coalesce((p_fixed->>'efectivo_usd')::numeric, 0) > 0
    );
$$;
...
create or replace function public._sale_unit_usd_fixed(
  p_fixed jsonb,
...
immutable
as $$
  -- Dart: f.efectivoUsd ?? product.precioUsd
  select coalesce(
    (p_fixed->>'efectivo_usd')::numeric,
    p_precio_usd,
    0
  );
$$;

revoke all on function public._sale_has_fixed_prices(jsonb) from public, anon, authenticated;
revoke all on function public._sale_unit_ars_fixed(jsonb, text) from public, anon, authenticated;
revoke all on function public._sale_unit_usd_fixed(jsonb, numeric) from public, anon, authenticated;
