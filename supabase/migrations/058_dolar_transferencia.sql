-- =====================================================================
-- 058_dolar_transferencia.sql
-- ---------------------------------------------------------------------
-- Nuevo medio de pago `dolar_transferencia` (USD por transferencia).
-- Cotiza EXACTAMENTE igual que `dolar_billete` (USD efectivo): mismo
-- monto en USD, misma conversión ARS/tipo de cambio. La única diferencia
-- es el registro: se guarda con su propia clave para poder distinguir en
-- el comprobante y en los reportes si los dólares entraron como billete o
-- por transferencia.
--
-- Estrategia: en todas las ramas de PRICING donde se trata `dolar_billete`
-- se acepta también `dolar_transferencia`; el método original se conserva
-- tal cual en la línea guardada (`paymentMethod`) y en las allocations.
-- =====================================================================

begin;

-- ---------------------------------------------------------------------
-- Helper dinámico: USD (billete o transferencia) = mismo % que efectivo.
-- (Copia de 050_wg + rama dolar_transferencia.)
-- ---------------------------------------------------------------------
...
begin
  ...
  return case v_method
    ...
    when 'dolar_transferencia' then v_lista * (1 - v_efectivo / 100)
...
-- ---------------------------------------------------------------------
-- Helper de precios fijos (Urban/Rossi): USD transferencia = efectivo_ars.
-- (Copia de 042 + rama dolar_transferencia.)
-- ---------------------------------------------------------------------
create or replace function public._sale_unit_ars_fixed(
  p_fixed jsonb,
  p_method text
) returns numeric
language sql
immutable
as $$
  select case coalesce(nullif(trim(p_method), ''), 'lista')
    when 'efectivo' then coalesce((p_fixed->>'efectivo_ars')::numeric, 0)
    when 'transferencia' then coalesce((p_fixed->>'efectivo_ars')::numeric, 0)
    when 'lista' then coalesce((p_fixed->>'efectivo_ars')::numeric, 0)
    when 'dolar_billete' then coalesce((p_fixed->>'efectivo_ars')::numeric, 0)
    when 'dolar_transferencia' then coalesce((p_fixed->>'efectivo_ars')::numeric, 0)
    when 'tarjeta1' then coalesce((p_fixed->>'tarjeta_ars')::numeric, 0)
    when 'tarjeta3' then coalesce((p_fixed->>'cuota3_ars')::numeric, 0) * 3
    when 'tarjeta6' then coalesce((p_fixed->>'cuota6_ars')::numeric, 0) * 6
    when 'tarjeta12' then coalesce((p_fixed->>'cuota12_ars')::numeric, 0) * 12
    when 'debito' then 0
    when 'tarjeta9' then 0
    when 'tarjeta18' then 0
    else coalesce((p_fixed->>'efectivo_ars')::numeric, 0)
  end;
$$;

revoke all on function public._sale_unit_ars_fixed(jsonb, text) from public, anon, authenticated;

-- ---------------------------------------------------------------------
-- register_sale: acepta dolar_transferencia como USD (igual que billete).
-- (Copia de 050_register_sale_per_line_payment + IN (...) en cada rama USD.)
-- ---------------------------------------------------------------------
...
declare
  ...
  v_lines_usd_total numeric := 0;
  v_lines_ars_total numeric := 0;
  v_total_usd numeric := 0;
  v_total_ars numeric := 0;
  v_total_base numeric := 0;
  v_per_line boolean;
  v_alloc_out jsonb := '[]'::jsonb;
...
begin
  ...
  v_per_line := coalesce((p_items->>'perLinePayment')::boolean, false);
  ...
  loop
    ...
    if public._sale_has_fixed_prices(v_producto.fixed_prices) then
      ...
      -- USD (billete o transferencia) = mismo descuento que efectivo.
      if v_method in ('dolar_billete', 'dolar_transferencia') and v_rate > 0 then
    ...
    -- Suma por moneda del propio método de la línea (pago por producto).
    if v_method in ('dolar_billete', 'dolar_transferencia') then
      v_lines_usd_total := v_lines_usd_total + v_line_usd;
    else
      v_lines_ars_total := v_lines_ars_total + v_line_ars;
    end if;
  ...
  if v_per_line then
    -- Cada línea aportó a su propia moneda: total = suma directa (sin share).
    v_total_usd := round(v_lines_usd_total, 4);
    v_total_ars := round(v_lines_ars_total, 2);
    v_total_base := v_lines_ars_total + v_lines_usd_total * v_rate;

    select coalesce(jsonb_agg(
             jsonb_build_object(
               'method', grouped.method_key,
               'amountUsd', round(grouped.amt_usd, 4),
               'amountArs', round(grouped.amt_ars, 2),
               'share', case
                 when v_total_base > 0
                   then round((grouped.amt_ars + grouped.amt_usd * v_rate) / v_total_base, 6)
                 else 0
               end
             ) order by grouped.ord
           ), '[]'::jsonb)
      into v_alloc_out
      from (
        select
          (elems.value->>'paymentMethod') as method_key,
          min(elems.ord) as ord,
          sum(case when (elems.value->>'paymentMethod') in ('dolar_billete', 'dolar_transferencia')
                then (elems.value->>'lineUsd')::numeric else 0 end) as amt_usd,
          sum(case when (elems.value->>'paymentMethod') in ('dolar_billete', 'dolar_transferencia')
                then 0 else (elems.value->>'lineArs')::numeric end) as amt_ars
        from jsonb_array_elements(v_built_lines) with ordinality as elems(value, ord)
        group by (elems.value->>'paymentMethod')
      ) grouped;

    select array_agg(a->>'method') into v_method_keys
      from jsonb_array_elements(v_alloc_out) a;
  elsif jsonb_typeof(v_allocs) = 'array' and jsonb_array_length(v_allocs) > 0 then
    ...
    select coalesce(jsonb_agg(
      jsonb_build_object(
        ...
        'amountUsd', case
          when coalesce(nullif(trim(a->>'method'), ''), v_pricing_method) in ('dolar_billete', 'dolar_transferencia')
    ...
    if v_pricing_method in ('dolar_billete', 'dolar_transferencia') then
