-- =====================================================================
-- 047_pricing_municion_override.sql
-- ---------------------------------------------------------------------
-- Override de pricing por tipo (munición): p.ej. World Guns —
-- 10% efectivo/transferencia + 3 cuotas sin interés solo en munición.
--
-- Cliente: PricingSettingsService.municion* + PricingService.
-- Server: _sale_unit_ars lee p_pricing.municion cuando product_type=municion.
-- =====================================================================

begin;

-- Firma nueva (5º arg opcional). Drop de la vieja para evitar overload.
drop function if exists public._sale_unit_ars(numeric, numeric, text, jsonb);
...
begin
  ...
  -- Override munición: usar claves anidadas si vienen.
  if lower(coalesce(nullif(trim(p_product_type), ''), '')) = 'municion'
     and jsonb_typeof(v_src->'municion') = 'object' then
    v_efectivo := public._sale_clamp_pct(
      coalesce(
        (v_src->'municion'->>'efectivo')::numeric,
        (v_src->>'efectivo')::numeric
      ),
      5, 0, 30
    );
    v_t3 := public._sale_clamp_pct(
      coalesce(
        (v_src->'municion'->>'tarjeta3')::numeric,
        (v_src->>'tarjeta3')::numeric
      ),
      15, 0, 100
    );
    v_transfer_efectivo := coalesce(
      (v_src->'municion'->>'transferencia_como_efectivo')::boolean,
      false
    );
  else
  ...
  return case v_method
    ...
    else v_lista -- lista / dolar_billete (ARS ref)
  end;
end;
$$;

revoke all on function public._sale_unit_ars(numeric, numeric, text, jsonb, text)
  from public, anon, authenticated;

-- register_sale: pasar product_type a _sale_unit_ars (resto igual a 042).
...
-- Seed World Guns: promo munición (10% efectivo/transf + 3 SI).
-- Armas: defaults del tenant (5% efectivo, recargos habituales).
update public.app_config ac
 ...
 where ac.tenant_id = t.id
   ...
   and (
     ac.pricing_settings is null
     or ac.pricing_settings = '{}'::jsonb
     or not (ac.pricing_settings ? 'municion')
   );
