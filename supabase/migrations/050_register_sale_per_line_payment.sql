-- =====================================================================
-- 050_register_sale_per_line_payment.sql
-- ---------------------------------------------------------------------
-- Medio de pago POR PRODUCTO (promo + dos medios en un mismo comprobante).
-- Cuando el cliente manda `perLinePayment: true`, cada línea aporta a su
-- propia moneda y el total es la suma directa (no se parte un total por
-- porcentaje). Las allocations se arman agrupando las líneas por método,
-- con montos reales que el servidor recalcula (nunca confía en el cliente).
--
-- Compatibilidad: sin `perLinePayment`, el comportamiento es idéntico a 049
-- (método único = una moneda; dual = reparto por share).
-- =====================================================================
    ...
    select coalesce(jsonb_agg(
      ...
      from (
        select
          ...
          sum(case when (elems.value->>'paymentMethod') = 'dolar_billete'
